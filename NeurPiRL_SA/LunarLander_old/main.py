...
def main():
    ...
    parser.add_argument('-time', action='store', dest='time_limit', default=7200,
    ...
    folder_name = str(parameters.eval_function)[0] + str(int(parameters.bayes_opt)) + str(int(parameters.augment_dsl)) \
                  + parameters.approach + (
                      "" if parameters.init_program is None else ("_" + parameters.init_program[0:4])) \
                  + '/Oracle-' + parameters.oracle

    file_name = '_n-' + str(number_games) + '_c-' + str(parameters.capacity) + '_run-' + str(
        seed) + parameters.file_name
    ...
    if parameters.oracle is not None:
        ...
        oracle.qnetwork_local.load_state_dict(
            torch.load("../LunarLander/DQN/Oracle/" + parameters.oracle + '/Policy.pth'))
        # Load Trajectory
    ...
    if parameters.init_program is not None:
        parameters.init_program = pickle.load(open("../LunarLander/binary_programs/" + parameters.init_program +"/Oracle-" , "rb"))
