...
class Imitation(Evaluate):
    ...
    def __init__(self, oracle, *args):
        ...
        self.inputs = oracle.pop('inputs')
        self.actions = oracle.pop('actions')
...
class DAgger(Evaluate):
    ...
    def __init__(self, oracle, *args):
        ...
        self.worst_score = 0.0
        self.oracle = oracle.pop('oracle')
        self.capacity = oracle.pop('capacity')
        # Histories
        if self.capacity is None:
            self.inputs = oracle.pop('inputs')
            self.actions = oracle.pop('actions')
        else:
            self.inputs = deque(oracle.pop('inputs'), maxlen=int(self.capacity))
            self.actions = deque(oracle.pop('actions'), maxlen=int(self.capacity))
    ...
    def evaluate(self, p):
        ...
        return 1.0 - actions_diff, self.nb_evaluations
...
class DAggerQ(Evaluate):
    ...
    def __init__(self, oracle, *args):
        ...
        self.worst_score = 0.0
        self.oracle = oracle.pop('oracle')
        self.inputs = deque(oracle.pop('oracle'), maxlen=50000)
        self.actions = deque(oracle.pop('oracle'), maxlen=50000)
