...
def main(seed=0, l1_actor=4, l2_actor=8):
    ...
    action_noise = -2 #env.env.state[0]

    # train oracle
    model = DDPG('MlpPolicy', env, verbose=3, policy_kwargs=dict(activation_fn=torch.nn.ReLU, net_arch=net_arch), action_noise=action_noise)
    model.learn(int(1e6))
