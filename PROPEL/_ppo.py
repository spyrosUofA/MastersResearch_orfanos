...
def main():
    ...
    action_space = 1 # np.arange(env.action_space.n)
    ...
    for steps in range(num_steps):
        ...
        # Select an action
        ####### Start
        a = actor(torch.FloatTensor(o))
        #print(a)
        a_mixed = mix_l * a.detach() + (1-mix_l) * my_program(o, reg_tree)
        mixed_action.append(a_mixed)
        #print(a_mixed)
        ####### End

        # Observe
        op, r, done, infos = env.step([a_mixed])
        ...
        if done:
            ...
            if k % K == 0:
                ...
                reg_tree = tree.DecisionTreeRegressor(max_depth=2, random_state=seed)

                #print(s_batch)

                #print(a_batch)
                #print(torch.stack(a_batch).detach().numpy())
                #reg_tree.fit(s_batch, torch.stack(a_batch).detach().numpy())
                reg_tree.fit(s_batch, mixed_action)
                ...
                policy_prime = actor(torch.FloatTensor(s_batch)) #torch.gather(actor(torch.FloatTensor(s_batch)), 1, a_batch.unsqueeze(1)).squeeze()
               ...
               # print(policy_prime)
                ...
                for epoch in range(E):
                    ...
                    for mini in range(len(mini_batch_indx)):
                        ...
                        shuffled_policy = actor(torch.FloatTensor(shuffled_states)) #torch.gather(actor(torch.FloatTensor(shuffled_states)), 1, shuffled_actions.unsqueeze(1)).squeeze()
