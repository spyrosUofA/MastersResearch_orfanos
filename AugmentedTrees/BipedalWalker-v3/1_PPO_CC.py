...
def train_trees(x, y, depth, seed):
    trees = []
    for i in range(4):
        regr_tree = tree.DecisionTreeRegressor(max_depth=depth).fit(x, y[i])
    ...
    for i in range(rollouts):
        ...
        for i in range(eps_per_rollout):
            ...
            while not done:
                ...
                a_star = model.predict(ob, deterministic=True)[0]
                for i in range(4):
                    Y[i].append(a_star[i])
                # Interact with Environment
...
def main():
    ...
    parser.add_argument('-l1_actor', action='store', dest='l1_actor', default=256)
    parser.add_argument('-l2_actor', action='store', dest='l2_actor', default=256)
    parameters = parser.parse_args()

    seed = int(parameters.seed)
    l1_actor = int(parameters.l1_actor)
    l2_actor = int(parameters.l2_actor)
    ...
    for depth in range(3, 4):
        ...
        np.save(file=save_to + 'AugTreeRewards' + str(depth) + '.npy', arr=rewards)
        pickle.dump(programs, file=open(save_to + 'AugTreePrograms' + str(depth) + '.pkl', "wb"))
    return
