...
def augmented_dagger(env, model, save_to, depth, rollouts, eps_per_rollout, seed):
    ...
    X = np.load(save_to + "Neurons_" + str(eps_per_rollout+20) + ".npy").tolist()
    Y = np.load(save_to + "Actions_" + str(eps_per_rollout+20) + ".npy").tolist()
    relu_programs = pickle.load(open(save_to + "ReLUs.pkl", "rb"))
    best_reward = -10000
...
def main(seed, l1_actor, l2_actor):
    ...
    for depth in range(2, 5):
        reward, program = augmented_dagger(env, model, save_to, depth, 25, 5, seed)
