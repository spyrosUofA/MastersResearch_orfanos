...
def main():
    ...
    if parameters.oracle is not None:
        ...
        print(len(actions))
        # Load ReLUs
        accepted_relus = pickle.load(open("../CartPole/Oracle/" + parameters.oracle + "/ReLUs.pkl", "rb"))
    ...
    if parameters.init_program is not None:
        parameters.init_program = pickle.load(open("../CartPole/binary_programs/" + parameters.init_program, "rb"))
    ...
    if isinstance(algorithm, SimulatedAnnealing):
        ...
        #OPERATIONS = [AssignAction, Ite, Lt, Lt0, Num, Observation, Addition, Multiplication]

        # Add ReLU node to DSL and load ReLU programs
        if parameters.augment_dsl:
            print("HELLO")
            OPERATIONS = [AssignAction, ReLU, Ite, Lt0, Gt0, Num, Observation] #, Addition, Multiplication]
