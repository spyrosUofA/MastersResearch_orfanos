...
model = PPO('MlpPolicy', env, seed=seed, verbose=0, policy_kwargs=dict(activation_fn=torch.nn.ReLU, net_arch=[dict(pi=[8], vf=[128, 128])]))
...
if policy_fn:
    ...
    print(outputs)
    print("SoftMax of otputs: ", softmax(outputs))
    print("Log SoftMax", np.log(softmax(outputs)))

    values, log_prob, entropy = model.policy.evaluate_actions(obs_tensor, actions)
    print("Official log_prog", log_prob)
